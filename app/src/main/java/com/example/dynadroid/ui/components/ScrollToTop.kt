package com.example.dynadroid.ui.components

import androidx.compose.animation.core.animateDp
import androidx.compose.animation.core.updateTransition
import androidx.compose.foundation.layout.height
import androidx.compose.foundation.layout.offset
import androidx.compose.material.icons.Icons
import androidx.compose.material.icons.filled.ArrowUpward
import androidx.compose.material3.ExtendedFloatingActionButton
import androidx.compose.material3.Icon
import androidx.compose.material3.Text
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.res.stringResource
import androidx.compose.runtime.getValue
import androidx.compose.ui.tooling.preview.Preview
import androidx.compose.ui.unit.dp
import com.example.dynadroid.R
import com.example.dynadroid.ui.theme.OffWhite
import com.example.dynadroid.ui.theme.PrimaryBlueVariant

private enum class Visibility {
    VISIBLE,
    GONE,
}


@Composable
fun ScrollToTop(enabled: Boolean, onClicked: () -> Unit, modifier: Modifier = Modifier) {
    val updateTransition = updateTransition(
        if (enabled) Visibility.VISIBLE else Visibility.GONE,
        label = "JumpToTop visibility animation"
    )

    val bottomOffset by updateTransition.animateDp(label = "JumpToTop visibility animation") {
        if (it == Visibility.GONE)
            (-32).dp
        else 32.dp
    }
    if (bottomOffset > 0.dp) {
        ExtendedFloatingActionButton(
            icon = {
                Icon(
                    imageVector = Icons.Filled.ArrowUpward,
                    modifier = Modifier.height(18.dp),
                    contentDescription = null
                )
            },
            text = {
                Text(text = stringResource(R.string.jump_to_top))
            }, onClick = onClicked,
            containerColor = PrimaryBlueVariant,
            contentColor = OffWhite,
            modifier = modifier
                .offset(x = 0.dp, y = -bottomOffset)
                .height(36.dp)
        )
    }
}

@Preview
@Composable
private fun ScrollToTopPrev() {
    ScrollToTop(
        enabled = false, onClicked = {}
    )
}